...
This fork of `bluesky-social/atproto` carries laugh.town-specific PDS changes (handle
reservation, invite-to-claim signup, custom emails — all in `packages/pds/`) and ships them
to the bag box (`bag.laugh.town`) via an event-driven pipeline.
...
The image build uses the upstream `services/pds/Dockerfile` unchanged — no custom Dockerfile.
...
`upstream` remote points at `bluesky-social/atproto`. Rebase laugh.town changes onto upstream
periodically; keeping edits confined to `packages/pds/` makes those rebases cheap.
